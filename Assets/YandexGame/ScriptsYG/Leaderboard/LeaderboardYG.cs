...
namespace YG
{
    ...
    public class LeaderboardYG : MonoBehaviour
    {
        ...
        [Tooltip("Кол-во получения верхних топ игроков")] [Range(1, 20)]
        public int quantityTop = 3;

        [Tooltip("Кол-во получаемых записей возле пользователя")] [Range(1, 10)]
        ...
        [SerializeField, ConditionallyVisible(nameof(advanced)),
         Tooltip("Родительский объект для спавна в нём объектов 'playerDataPrefab'")]
        private Transform rootSpawnPlayersData;

        [SerializeField, ConditionallyVisible(nameof(advanced))]
        private Transform _rootSpawnPlayer;

        [ConditionallyVisible(nameof(advanced)),
         Tooltip("Префаб отображаемых данных игрока (объект со компонентом LBPlayerDataYG)")]
        public GameObject playerDataPrefab;

        [SerializeField] private Image _currentName;
        [SerializeField] private Sprite _spriteNameRu;
        [SerializeField] private Sprite _spriteNameEn;
        [SerializeField] private Sprite _spriteNameTr;
        ...
        [SerializeField] private UnityEvent onUpdateData;
            ...
            else if (lb.technoName == nameLB)
            {
                ...
                if (lb.entries == "no data")
                {
                    switch (YandexGame.savesData.language)
                    {
                        case "ru":
                            noData = "Нет данных";
                            _currentName.sprite = _spriteNameRu;
                            break;
                        case "en":
                            noData = "No data";
                            _currentName.sprite = _spriteNameEn;
                            break;
                        case "tr":
                            noData = "Veri yok";
                            _currentName.sprite = _spriteNameTr;
                            break;
                        default:
                            noData = "...";
                            break;
                ...
                if (!advanced)
                {
                    lb.entries = lb.entries.Replace("anonymous",
